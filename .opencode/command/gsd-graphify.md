...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node /root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node /root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node /root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node /root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs graphify diff
```
...
```
PREFLIGHT=$(node "/root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs" graphify build)
```
...
Project root: ${CWD}
gsd-tools path: /root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs
...
4. **Write diff snapshot:**
   ```
   node \"/root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs\" graphify build snapshot
   ```
