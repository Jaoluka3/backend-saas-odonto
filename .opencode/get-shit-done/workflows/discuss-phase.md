...
<required_reading>
@/root/meu-backend/.opencode/get-shit-done/references/domain-probes.md
@/root/meu-backend/.opencode/get-shit-done/references/gate-prompts.md
@/root/meu-backend/.opencode/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "/root/meu-backend/.opencode/get-shit-done/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@/root/meu-backend/.opencode/get-shit-done/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
