...
- **Global scope**: `/root/meu-backend/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/root/meu-backend/.opencode/notes/`)
...
2. Glob `/root/meu-backend/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (/root/meu-backend/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/root/meu-backend/.opencode/notes/` — works in any directory
