...
```bash
INIT=$(node "/root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs" init ingest-docs)
...
```bash
node "/root/meu-backend/.opencode/get-shit-done/bin/gsd-tools.cjs" commit \
